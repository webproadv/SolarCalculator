...
// Mercator usata da Google Maps (formula standard Maps Platform).
...
// richiesta dataLayers, qui applicata allo zoom della Static Maps API).
function zoomPerRaggio(latDeg, radiusMeters, imageWidthPx) {
  const metriPerPixelDesiderati = (radiusMeters * 2) / imageWidthPx;
  const zoomGrezzo = Math.log2(156543.03392 * Math.cos((latDeg * Math.PI) / 180) / metriPerPixelDesiderati);
  return Math.min(20, Math.max(15, Math.round(zoomGrezzo)));
}

// Genera la foto aerea del sito tramite la Google Maps Static API (vista
// satellitare), inquadrata sulle coordinate dell'azienda.
//
// Sostituisce la versione precedente basata sul layer RGB della Google Solar
// API (dataLayers): quella richiesta è nel livello "Enterprise" della Solar
// API ed è disponibile solo dove Google ha fatto un rilievo dettagliato
// dell'edificio, quindi falliva spesso ("nessun layer RGB disponibile") anche
// per indirizzi normalmente coperti da Google Maps. Dato che l'app non
// disegna più i pannelli sovrapposti alla foto (vedi nota storica sotto), una
// semplice foto satellitare centrata sul sito è sufficiente ed è disponibile
// ovunque Google Maps abbia copertura satellitare — praticamente ogni
// indirizzo, incluso quando buildingInsights non ha un rilievo del tetto
// (roofDataUnavailable).
//
// NOTA STORICA: una versione precedente generava anche una foto con la
...
//
// Usa la stessa chiave della Solar API/Geocoding: va abilitata anche la Maps
// Static API sullo stesso progetto/chiave.
export async function fetchRoofImages(lat, lng, { segments } = {}) {
  ...
  const url =
    ...
    `&size=${width}x${height}&maptype=satellite&key=${apiKey}`;
  const res = await fetch(url, { cache: "no-store" });
  if (!res.ok) throw new Error(`Google Maps Static API ${res.status}: ${await res.text()}`);
